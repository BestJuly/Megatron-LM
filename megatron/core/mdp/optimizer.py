...
def build_encoder_optimizer_config(decoder_config):
    """Keep BF16 encoder updates synchronous and independent of MXFP8 staging."""
    if getattr(decoder_config, "fp8_recipe", None) != "mxfp8":
...
class MdpChainedOptimizer(ChainedOptimizer):
    ...
    def step_with_ready_grads(self) -> bool:
        """Keep native decoder MXFP8 staging separate from synchronous encoder updates."""
    ...
    def prepare_model_params_for_param_sync(self) -> None:
        """Reset shared decoder buffers only once; encoder buffers are not shared."""
        if self._decoder_chain is None:
...
def build_mdp_composite_optimizer(
    ...
    sequence the design specifies. Members share optimizer hyperparameters;
    the BF16 encoder projects out decoder-only MXFP8 staging options.
    """
