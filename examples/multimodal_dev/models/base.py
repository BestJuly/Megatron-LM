    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.pre_process:
            ...
            # set_input_tensor, already CP-split inside the upstream
            # stage.  Don't run vision encoder, don't run embedding, and
            # don't double-CP-split the activation below.
            decoder_input = None

        # Per-token tensors (input_ids, labels, loss_mask, attention_mask)
        # must be CP-split on every stage that consumes them so that the
        # last stage's loss aligns with the CP-split logits.  When
        # decoder_input is None (non-first stages), the helper leaves it
        # as None.
        (
            decoder_input, input_ids, labels, loss_mask,
            attention_mask, position_ids,
