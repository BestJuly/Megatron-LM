...
# Launch script for Kimi K2.5 VL MIMO training.
#
# Usage (from the Megatron-LM repo root):
#   # Quick proxy test (2 GPUs, no real data needed):
#   MODEL_VARIANT=proxy GPUS_PER_NODE=2 EP=2 \
#       ./examples/mimo/scripts/run_kimi_k25_vlm_train.sh
#
#   # Proxy with explicit mock data:
#   MODEL_VARIANT=proxy GPUS_PER_NODE=2 EP=2 \
#       ./examples/mimo/scripts/run_kimi_k25_vlm_train.sh mock
#
#   # Full production run:
#   ./examples/mimo/scripts/run_kimi_k25_vlm_train.sh /path/to/dataset [/path/to/llm/ckpt]
#
...
#   KIMI_K25_HF_MODEL_PATH  Path or HF hub ID for the Kimi K2.5 VL model
#                            (default: moonshotai/Kimi-K2.5)
#   PROFILE=1               Enable Nsight Systems profiling (default: 0)
#   PROFILE_STEP_START/END  Profiled iteration window (default: 4-5)
#
...
WANDB_PROJECT='mimo-kimi-k25-vlm'
EXP_NAME="kimi_k25_${MODEL_VARIANT}_mbs_${MBS}_gbs_${GBS}_tp${TP}_ep${EP}"
...
TOKENIZER_ARGS=(
    ...
    --tokenizer-model 'moonshotai/Kimi-K2.5'
)
...
# --- Kimi K2 MoE+MLA Decoder Architecture ---
# Architecture values come from configuration.py variants.
# CLI args here provide the MCore-level flags that get_args() reads.
if [ "$MODEL_VARIANT" = "proxy" ]; then
    GPT_MODEL_ARGS=(
        --num-layers 4
        --hidden-size 4096
...
echo "================================================================"
echo "Kimi K2.5 VL MIMO Training  [variant: $MODEL_VARIANT]"
