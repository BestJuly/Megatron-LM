...
"""Forward step and data loading for multimodal training."""
...
def _broadcast_tensor(tensor, src, group, device):
    """Broadcast a tensor from src rank to all ranks in group."""
...
def broadcast_data_batch(data, device="cuda"):
    ...
    Handles nested tensors. Only TP rank 0 should provide non-None data.
    """
...
def get_batch(data_iterator: Iterator[Dict[str, Any]]):
    """Get a batch from the data iterator and broadcast across TP ranks.
...
def loss_func(loss_mask, output_tensor):
    ...
    Args:
        loss_mask: [B, S] mask for which tokens contribute to loss.
        output_tensor: Per-token losses from the model.

    Returns:
        (total_loss, num_tokens, metrics_dict)
    """
...
def forward_step(data_iterator, model):
    """Forward step for multimodal model training.
    ...
    # Compute position IDs if the model supports it
