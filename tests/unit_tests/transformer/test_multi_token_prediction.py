...
class TestMultiTokenPredictionLayer:
    ...
    def test_shift_decoder_input_context_parallel(self, cp):
        ...
        Embeddings need the gradient of a boundary token to reach the CP rank that owns it,
        so the boundary exchange must be differentiable.
        """
    ...
    def test_shift_decoder_input_packed_sequences_contiguous_cp(self):
        """Contiguous CP shifts embeddings across ranks and preserves remote gradients."""
        cp = 2
        config, mtp_block_spec = self._create_config_and_mtp_block_spec(
            tp=1, cp=cp, attention_dropout=0.0
        )
        ...
        global_input = _sequence_first_arange(boundaries[-1], 1, config.hidden_size)
        local_seq_len = global_input.shape[0] // cp
        local_input = (
            global_input.narrow(0, cp_rank * local_seq_len, local_seq_len)
            .clone()
            .requires_grad_(True)
        )
        ...
        expected = expected_global.narrow(0, cp_rank * local_seq_len, local_seq_len)
        assert torch.equal(shifted, expected)
        ...
        for start in boundaries[:-1]:
            expected_global_grad[start].zero_()
        expected_grad = expected_global_grad.narrow(0, cp_rank * local_seq_len, local_seq_len)
        assert torch.equal(local_input.grad, expected_grad)
