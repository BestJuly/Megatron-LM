...
# materialization on non-owner workers. ``None`` outside MDP window capture, so
# the native path is unchanged. Single-threaded by design at this point in the
# history; window-capture prefetch is introduced by the later performance commit.
